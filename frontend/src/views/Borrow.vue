<template>
  <div class="bg-white rounded-lg shadow-md p-4 md:p-6">
    <h2 class="text-xl md:text-2xl font-bold text-gray-800 mb-4 md:mb-6">我的借阅</h2>
          ...
          <tr v-for="record in records" :key="record.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm">
              <button
                ...
                class="px-3 py-1 bg-orange-500 text-white rounded hover:bg-orange-600 transition mr-2 text-xs"
              >
                归还
              </button>
              <button
                v-if="record.renewCount < 3"
    ...
    <div class="md:hidden space-y-4">
      ...
      >
        <h4 class="font-semibold text-gray-800 text-sm mb-2">{{ record.bookTitle }}</h4>
        ...
        <div class="flex flex-wrap gap-2">
          <button
            @click="handleReturn(record.id)"
            class="px-3 py-1 bg-orange-500 text-white rounded hover:bg-orange-600 transition text-xs"
          >
            归还
          </button>
          <button
...
const loadRecords = async () => {
  try {
    const response = await borrowApi.getMyBorrows(page.value, size.value)
